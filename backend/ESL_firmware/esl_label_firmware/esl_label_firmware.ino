...
const char* LEGACY_DEFAULT_WIFI_SSID = "your_wifi_ssid";
const char* LEGACY_DEFAULT_WIFI_PASS = "your_wifi_password";
const char* LEGACY_DEFAULT_BACKEND_IP = "192.168.1.100";
...
static void pauseSerialForDisplayRefresh()
{
  Serial.flush();
  delay(20);
  Serial.end();
...
static void resumeSerialAfterDisplayRefresh()
{
  delay(20);
...
static void askNetworkConfigFromSerial()
{
  ...
  Serial.print("- Token: "); Serial.println(eslDeviceToken.length() > 0 ? "ayarli" : "yok");
                        ...
                        const String& code)
{
  ...
  const int16_t quietPx = 8;
  int16_t barAreaW = w - quietPx * 2;
  if (barAreaW < total) barAreaW = total;
  int16_t startX = x + max((int16_t)0, (int16_t)((w - barAreaW) / 2));

  for (int i = 0; i < total; i++) {
    if (mods[i] == '1') {
      int16_t bx = startX + (int32_t)i * barAreaW / total;
      int16_t nextX = startX + (int32_t)(i + 1) * barAreaW / total;
      int16_t bw = max((int16_t)1, (int16_t)(nextX - bx));
      display.fillRect(bx, y, bw, barH, GxEPD_BLACK);
    }
  }
...
static void drawStandardLayout(const LabelData& data)
{
  ...
  int16_t titleBottomY;
  {
    ...
    for (int i = 0; i < lc; i++) {
      int16_t lineOx, lineOy;
      uint16_t lineW, lineH;
      measure(lines[i], titleFont, lineOx, lineOy, lineW, lineH);
      display.setCursor(fX + IP - lineOx, fY + IP + titleH + i * (titleH + 3));
  ...
  // --- Menşei + FDT (barkodun tam üstünde, aralarında boşluklu) ---
  {
    ...
    int16_t blockBottomY = bcY - 1; // Barkod ile mensei arasinda 4px civari bosluk kalir
    int16_t originH = 10; // Mensei satiri yuksekligi (font 1 icin yaklasik)

    int16_t originY = blockBottomY - originH;
    ...
    // Menşei
    int16_t originTW = origin.length() * 6;
    int16_t originX = bcX + (bcW - originTW) / 2;
    if (originX < bcX) originX = bcX;
  ...
  // FDT siyah fiyat kutusunun hemen ustunde, KDV Dahil ile ayni merkez hizada.
  {
    String fdtText = fdt.length() == 0 ? "FDT:../../...." : fdt;
    display.setFont(nullptr);
    display.setTextSize(1);
    display.setTextColor(GxEPD_BLACK);
    int16_t fdtTW = fdtText.length() * 6;
    int16_t fdtX = priceX + (priceW - fdtTW) / 2;
    if (fdtX < priceX + 2) fdtX = priceX + 2;
    display.setCursor(fdtX, priceY - 9);
...
bool fetchLabel()
{
  ...
  Serial.println(normalizeTrToAscii(response));
  http.end();
  ...
  Serial.print("Urun: ");               Serial.println(normalizeTrToAscii(currentLabel.product));
  ...
  Serial.print("Mensei: ");              Serial.println(normalizeTrToAscii(currentLabel.origin));
...
bool sendRenderConfirm(const String& status, const String& errorMessage)
{
  ...
  if (httpCode != 200) {
    ...
    if (httpCode <= 0) {
      ...
      Serial.println(normalizeTrToAscii(http.getString()));
    }
    http.end();
    return false;
  }
...
bool updateDisplay()
{
  ...
  Serial.println(normalizeTrToAscii(currentLabel.product));
  Serial.print("Barcode: ");
  Serial.println(currentLabel.barcode);

  pauseSerialForDisplayRefresh();
  ...
  display.powerOff();
  resumeSerialAfterDisplayRefresh();
