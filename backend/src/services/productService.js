...
import { sanitizeProductInput, validateProductPayload, normalizeSearchText } from '../utils/validators.js';
...
const toProductUniqueMessage = (error) => {
  if (String(error?.code || '') !== 'P2002' && !/unique constraint|duplicate key/i.test(String(error?.message || ''))) {
    return '';
  }
  const target = Array.isArray(error?.meta?.target) ? error.meta.target.join(' ') : String(error?.meta?.target || error?.message || '');
  return /barcode/i.test(target)
    ? 'Bu barkod bu tenant içinde zaten kullanılıyor.'
    : 'Bu SKU bu tenant içinde zaten kullanılıyor.';
};

const persistProductWrite = async (callback) => {
  try {
    return await callback();
  } catch (error) {
    const message = toProductUniqueMessage(error);
...
const buildProductWhere = ({
  ...
  if (q) {
    ...
    conditions.push({
      OR: [
        ...
        { category: { is: { normalizedSearch: { contains: normalizedQ } } } },
        { supplier: { is: { normalizedSearch: { contains: normalizedQ } } } },
        {
          supplierProducts: {
            some: {
              OR: [
  ...
  if (supplierName) {
    const normalizedSupplierName = normalizeSearchText(supplierName);
    conditions.push({ supplier: { is: { normalizedSearch: { contains: normalizedSupplierName } } } });
  }
...
export const productService = {
  ...
  async create(payload) {
    ...
    await persistProductWrite(() => productRepo.create(product));
  ...
  async update(id, payload) {
    ...
    if (payload?.rejectDraft === true) {
      ...
      await persistProductWrite(() => productRepo.updateById(id, rejected));
