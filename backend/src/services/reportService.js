  ...
  try {
    const [
      ...
      prisma.accessAuditLog.findMany({
        orderBy: [{ createdAt: 'desc' }, { id: 'desc' }],
        take: 60,
        select: { id: true, action: true, userId: true, permission: true, requestId: true, actorId: true, metadata: true, createdAt: true },
      }),
      ...
      prisma.eslHistory.findMany({
        orderBy: [{ createdAt: 'desc' }, { id: 'desc' }],
        take: 40,
        select: { id: true, deviceId: true, deviceName: true, productId: true, productName: true, status: true, template: true, customFields: true, payload: true, createdAt: true },
      }),
      prisma.productPriceEvent.findMany({
        orderBy: [{ createdAt: 'desc' }, { id: 'desc' }],
        take: 40,
        select: { id: true, productId: true, source: true, payload: true, createdAt: true, product: { select: { name: true, sku: true } } },
      }),
...
const buildFastDashboardReport = async () => {
  const prisma = await getPrisma();
  ...
  const [
    ...
    prisma.$queryRaw`
      ...
      FROM categories c
      LEFT JOIN products p ON p.category_id = c.id AND COALESCE(p.is_active, true) = true
      LEFT JOIN stocks s ON s.product_id = p.id
...
const buildPriceCatalogDiffReportFromPriceEvents = async ({ query = {}, supplierProducts = [], supplierNameById = new Map() } = {}) => {
  ...
  const aggregatedRows = await prisma.$queryRaw`
    ...
    WHERE (${dateRange.startAt}::timestamp IS NULL OR e.created_at >= ${dateRange.startAt})
...
export const reportService = {
  async getPricingAnalysis(query = {}) {
    return pricingAnalysisService.getAnalysis(query);
  },
  ...
  async getDashboardSummary() {
    if (config.dataStore === 'postgres') {
      const now = Date.now();
      if (dashboardSummaryCache && now - dashboardSummaryCache.createdAt < DASHBOARD_CACHE_TTL_MS) {
        return {
      ...
      const data = await buildFastDashboardReport();
      dashboardSummaryCache = { createdAt: now, data };
      return {
        ...data,
  ...
  async getSummaryReport(query = {}) {
    if (config.dataStore === 'postgres' && !parseDetailsFlag(query.includeDetails)) {
      return buildFastDashboardReport();
    }
