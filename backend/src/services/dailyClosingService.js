  ...
  const upsertWithRawSql = async () => {
    const rows = await prisma.$queryRaw`
      INSERT INTO daily_store_closings (
        id, tenant_id, store_id, business_date, closing_type, closed_at, timezone, sales_count, return_count, transaction_count,
        gross_sales_amount, return_amount, net_revenue, item_count, source, payload
      )
      VALUES (
        ${closing.id}, ${tenantId}, ${closing.storeId}, ${businessDate}, ${closing.closingType}, ${new Date(closing.closedAt)}, ${closing.timezone}, ${closing.salesCount},
