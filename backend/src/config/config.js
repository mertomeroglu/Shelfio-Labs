...
const exportStorageDir = path.resolve(
  process.env.EXPORT_STORAGE_DIR || path.join(backendRootDir, 'storage', 'tenant-exports')
);
const normalizeUrl = (value, fallback) => String(value || fallback || '').trim().replace(/\/+$/, '');
const DEFAULT_PUBLIC_MAIN_APP_URL = 'https://shelfiolabs.com';
const publicAppBaseUrl = normalizeUrl(process.env.PUBLIC_APP_BASE_URL || process.env.FRONTEND_BASE_URL, DEFAULT_PUBLIC_MAIN_APP_URL);
const normalizeLicenseMode = (value) => {
  const mode = String(value || 'off').trim().toLowerCase();
  return ['off', 'shadow', 'enforce'].includes(mode) ? mode : 'off';
};

export const resolvePublicApiBaseUrl = (env = process.env, runtimeNodeEnv = nodeEnv) => {
  const configured = normalizeUrl(
    env.PUBLIC_API_BASE_URL || env.MAIN_APP_URL || env.PUBLIC_APP_BASE_URL || env.FRONTEND_BASE_URL,
    '',
  );
  if (configured) return configured;

  const runtime = String(runtimeNodeEnv || '').trim().toLowerCase();
  if (runtime === 'production') return DEFAULT_PUBLIC_MAIN_APP_URL;

  return `http://localhost:${parsePort(env.PORT, 4000)}`;
};
...
export const config = {
  ...
  supportUploadDir,
  exportStorageDir,
  exportDownloadTtlHours: parsePort(process.env.EXPORT_DOWNLOAD_TTL_HOURS, 24),
  ...
  shelfioLabsUsageSecret: process.env.SHELFIO_LABS_USAGE_SECRET || '',
  ...
  personnelDefaultLicenseKey: process.env.PERSONEL_DEFAULT_LICENSE_KEY || process.env.MAIN_SYSTEM_LICENSE_KEY || 'SHELFIO-MAIN-2026',
  shelfioPublicSiteUrl: normalizeUrl(process.env.SHELFIO_PUBLIC_SITE_URL, 'https://getshelfio.com'),
  mainAppUrl: normalizeUrl(process.env.MAIN_APP_URL, DEFAULT_PUBLIC_MAIN_APP_URL),
};
