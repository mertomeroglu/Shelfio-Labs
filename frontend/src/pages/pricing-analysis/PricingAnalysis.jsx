...
import { formatStorageTypeLabel, normalizeTurkishText, normalizeSearchText, includesNormalized } from '../../services/formatters.js';
...
function BulkPriceUpdateModal({
  ...
  const productSearchRows = useMemo(() => {
    if (form.productSearch.trim().length < 2) return [];
    return normalizedProducts
      .filter((product) => (
        includesNormalized(product.productName, form.productSearch)
        || includesNormalized(product.sku, form.productSearch)
        || includesNormalized(product.barcode, form.productSearch)
        || includesNormalized(product.categoryName, form.productSearch)
      ))
      .slice(0, 24);
                  ...
                  <div className="pricing-bulk-product-search-shell">
                    <label className="field-group">
                      ...
                      <input
                        ...
                        onFocus={() => setIsProductSearchOpen(form.productSearch.trim().length >= 2)}
                        onChange={(event) => {
                          const nextValue = event.target.value;
                          setForm((current) => ({ ...current, productSearch: nextValue }));
                          setIsProductSearchOpen(nextValue.trim().length >= 2);
                        }}
                    ...
                    {isProductSearchOpen ? (
                      <div className="pricing-bulk-product-popover" role="listbox" aria-label="Ürün seçimi">
                        {form.productSearch.trim().length < 2 ? (
...
function SellPriceAdvisorModal({
  ...
  const canSearchProducts = query.trim().length >= 2;
  const filteredRows = rows
    .filter((row) => !isCatalogUnlistedProduct(row))
    .filter((row) => {
      if (!canSearchProducts) return false;
      return [row.productName, row.sku, row.barcode].some((value) => includesNormalized(value, query));
    })
    .slice(0, 20);
