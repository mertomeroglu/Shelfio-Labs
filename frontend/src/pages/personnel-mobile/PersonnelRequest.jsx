...
export default function PersonnelRequest() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products
      .filter((item) => {
        const name = normalizeText(resolveProductName(item));
        const sku = normalizeText(item?.sku || '');
        const barcode = String(item?.barcode || '');
        return name.includes(needle) || sku.includes(needle) || barcode.includes(productSearch.trim());
      })
      .slice(0, 8);
  ...
  if (loading) return <div className="personnel-empty-state">Talep sistemi yükleniyor...</div>;
      ...
      {selectedProduct && (
        <section className="personnel-section-card" style={{ background: '#eff6ff', borderColor: '#bfdbfe' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
            ...
            <button type="button" className="ghost-button" style={{ minHeight: '32px', padding: '0 8px', color: '#64748b' }} onClick={() => setSelectedProductId('')}>
              <X size={18} />
            </button>
          </div>
