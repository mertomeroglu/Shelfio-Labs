...
export default function PersonnelLabels() {
  ...
  const { results: filteredProducts, isSearching: isProductSearching } = usePersonnelProductSearch(productSearch, { limit: 10 });
  ...
  const handleScanLookup = async () => {
    ...
    try {
      const productCandidates = selectedProduct ? [selectedProduct, ...filteredProducts] : filteredProducts;
      const result = await barcodeLookupService.resolveLabelScan(token, { products: productCandidates, devices });
      ...
      if (result.product?.id) setSelectedProduct(result.product);
    } catch (error) {
      setScanResult(null);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <section className="personnel-section-card">
        ...
        {isProductSearching && <span style={{ display: 'block', marginBottom: '8px', fontSize: '0.8rem', color: '#64748b' }}>Aranıyor...</span>}
        ...
        {filteredProducts.length > 0 && (
          <ul className="autocomplete-dropdown" style={{ position: 'relative', top: '0', boxShadow: 'none', border: '1px solid var(--p-border)', marginBottom: selectedProduct ? '16px' : '0' }}>
            {filteredProducts.map((item) => (
              <li key={item.id} onClick={() => { setSelectedProduct(item); setProductSearch(''); }} style={{ cursor: 'pointer' }}>
