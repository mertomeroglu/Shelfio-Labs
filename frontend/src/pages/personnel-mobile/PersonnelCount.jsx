...
export default function PersonnelCount() {
  ...
  const searchResults = useMemo(() => {
    const needle = normalizeQuery(searchQuery);
    if (needle.length < 2) return [];
    return products
      .filter((item) => (
        normalizeQuery(item?.productName).includes(needle)
        || normalizeQuery(item?.sku).includes(needle)
        || String(item?.barcode || '').includes(searchQuery.trim())
      ))
      .slice(0, 8);
  ...
  const handleLookupInputChange = useCallback((value) => {
    ...
    const exactBarcodeMatch = products.find((item) => String(item?.barcode || '').trim() === normalizedValue);
    if (exactBarcodeMatch) {
      applySelectedProduct(exactBarcodeMatch);
      return;
    }
    setSelectedProduct(null);
  }, [applySelectedProduct, products]);
  ...
  const startScanner = useCallback(async () => {
    ...
    try {
      ...
      const scanner = new Html5Qrcode('personnel-count-reader');
  ...
  const submitCount = useCallback(async () => {
    ...
    try {
      ...
      setProducts((current) => current.map((item) => (String(item.id) === String(refreshedProduct.id) ? refreshedProduct : item)));
