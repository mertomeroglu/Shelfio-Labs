...
export default function PersonnelMobile() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      setIsLoading(true);
      const [taskRows, productRows, activityRows] = await Promise.allSettled([
        taskService.list(),
      ...
      setProducts(productRows.status === 'fulfilled' && Array.isArray(productRows.value) ? productRows.value : []);
  ...
  const handleScan = useCallback(async () => {
    ...
    try {
      ...
      const scanner = new Html5Qrcode('personnel-quick-scan-reader');
        ...
        {searchResults.length > 0 && (
          <ul className="autocomplete-dropdown" style={{ position: 'relative', top: '0', boxShadow: 'none', border: '1px solid var(--p-border)' }}>
            {searchResults.map((item) => (
              <li key={item.id} onClick={() => { setSelectedProductId(item.id); setSearchQuery(''); }}>
        ...
        {selectedProduct && (
          <div style={{ marginTop: '16px', background: '#f8fafc', border: '1px solid var(--p-border)', borderRadius: '12px', padding: '16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
              ...
              <button type="button" className="ghost-button" style={{ minHeight: '32px', padding: '0 8px' }} onClick={() => setSelectedProductId('')}>
                <X size={18} />
              </button>
            </div>
