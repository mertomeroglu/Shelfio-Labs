...
const PERSONNEL_LOGIN_ERROR_MESSAGES = {
  user_inactive: 'Hesabınız pasif görünüyor.',
  personnel_access_denied: 'Bu kullanıcı personel girişi için yetkili değil.',
  personnel_license_unconfigured: 'Personel girişi şu anda yapılandırılamadı.',
};
...
export default function PersonnelLogin() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      setSubmitting(true);
      await login({ username: form.username.trim(), password: form.password, source: 'personnel_mobile' });
    } catch (requestError) {
      const errorCode = String(requestError?.payload?.errorCode || '').trim();
      if (requestError?.status === 401) {
        setError('E-posta veya şifre hatalı.');
      } else if (PERSONNEL_LOGIN_ERROR_MESSAGES[errorCode]) {
        setError(PERSONNEL_LOGIN_ERROR_MESSAGES[errorCode]);
      } else {
