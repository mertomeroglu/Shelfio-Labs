  ...
  const loadSettings = async () => {
    try {
      ...
      const [settingsResult, loginResult, auditResult, developerResult] = await Promise.allSettled([
        ...
        !isCampaignPage && isPlatformAdmin ? settingsService.getDeveloperLogs({ limit: 120 }) : Promise.resolve([]),
      ]);
  ...
  const handleClearLogRecords = async (type = activityLogTab) => {
    ...
    try {
      ...
      const [loginRows, auditRows, developerRows] = await Promise.all([
        ...
        isPlatformAdmin ? settingsService.getDeveloperLogs({ ...developerLogFilters, limit: 300 }) : Promise.resolve([]),
      ]);
      setLoginActivities(sanitizeObjectRows(loginRows));
      setLoginActivitiesTotal(extractResponseTotal(loginRows));
      setAuditLogs(sanitizeObjectRows(auditRows));
