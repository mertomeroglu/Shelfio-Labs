...
import { includesNormalized } from '../../services/formatters.js';
...
function getExpiryStatusLabel(value) {
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return '-';
  const now = new Date();
  const today = new Date(now.getFullYear(), now.getMonth(), now.getDate()).getTime();
  const day = new Date(date.getFullYear(), date.getMonth(), date.getDate()).getTime();
  if (day < today) return 'Geçmiş';
  if (day === today) return 'Bugün';
  return 'Yaklaşıyor';
}

function normalizeExpiryDetailRow(row = {}, index = 0) {
  const stock = row.stock && typeof row.stock === 'object' ? row.stock : {};
  const product = row.product && typeof row.product === 'object' ? row.product : (stock.product && typeof stock.product === 'object' ? stock.product : {});
  const batch = row.batch && typeof row.batch === 'object' ? row.batch : {};
  const summary = row.summary && typeof row.summary === 'object' ? row.summary : {};
  const details = row.details && typeof row.details === 'object' ? row.details : {};
  const rawStockValue = row.stock && typeof row.stock !== 'object' ? row.stock : undefined;
  const expiryDate = firstDefined(
    row.sktDate, row.expiryDate, row.skt, row.expirationDate, row.expiresAt, row.bestBeforeDate,
    batch.sktDate, batch.expiryDate, batch.skt, batch.expirationDate, batch.bestBeforeDate,
    details.sktDate, details.expiryDate, details.skt,
  );
  const warehouseStockValue = firstDefined(
    row.warehouseStock, row.depoStock, row.depotStock, row.storageStock, row.backroomStock,
    row.warehouseQuantity, row.depotQuantity, row.storageQuantity,
    stock.warehouseStock, stock.depoStock, stock.depotStock, stock.warehouseQuantity,
    batch.warehouseStock, batch.depoStock, batch.depotStock, batch.warehouseQuantity,
    summary.warehouseStock, summary.depoStock, summary.warehouseQuantity,
    details.warehouseStock, details.depoStock, details.warehouseQuantity,
  );
  const shelfStockValue = firstDefined(
    row.shelfStock, row.rayonStock, row.reyonStock, row.storefrontStock,
    row.shelfQuantity, row.rayonQuantity, row.reyonQuantity,
    stock.shelfStock, stock.rayonStock, stock.reyonStock, stock.shelfQuantity,
    batch.shelfStock, batch.rayonStock, batch.reyonStock, batch.shelfQuantity,
    summary.shelfStock, summary.rayonStock, summary.shelfQuantity,
    details.shelfStock, details.rayonStock, details.shelfQuantity,
  );
  const warehouseStock = toDisplayNumber(warehouseStockValue, 0);
  const shelfStock = toDisplayNumber(shelfStockValue, 0);
  const totalStockValue = firstDefined(
    row.totalStock, row.totalQuantity, row.quantity, row.qty, rawStockValue, row.stockQuantity, row.availableStock,
    stock.totalStock, stock.totalQuantity, stock.quantity, stock.qty, stock.availableStock, stock.onHand,
    batch.totalStock, batch.totalQuantity, batch.quantity, batch.qty, batch.availableStock,
    summary.totalStock, summary.totalQuantity, summary.quantity,
    details.totalStock, details.totalQuantity, details.quantity,
  );
  const totalStock = totalStockValue === undefined ? warehouseStock + shelfStock : toDisplayNumber(totalStockValue, warehouseStock + shelfStock);
  return {
    key: String(firstDefined(row.id, row.batchId, row.productBatchId, row.sourceKey, row.productId, row.sku, index)),
    productName: String(firstDefined(row.productName, row.name, row.title, product.name, product.productName, product.title, '-') || '-'),
    sku: String(firstDefined(row.sku, row.productSku, row.code, product.sku, product.productSku, product.code, '-') || '-'),
    barcode: String(firstDefined(row.barcode, row.productBarcode, row.ean, row.ean13, product.barcode, product.productBarcode, product.ean, product.ean13, '-') || '-'),
    batchNo: String(firstDefined(row.batchNo, row.lotNo, row.partyNo, row.batchCode, row.lotCode, row.partyCode, batch.batchNo, batch.lotNo, batch.partyNo, batch.batchCode, batch.lotCode, '-') || '-'),
    expiryDate: expiryDate ? formatDisplayDate(expiryDate) : '-',
    warehouseStock,
    shelfStock,
    totalStock,
    status: getExpiryStatusLabel(expiryDate),
  };
}

function getExpiryDetailSourceRows(item) {
  const payload = getNotificationPayload(item);
  const metadata = getNotificationMetadata(item);
  const groupItems = getNotificationGroupItems(item);
  const candidates = [
    payload.items, payload.products, payload.batches, payload.expiryItems, payload.expiredItems, payload.stockBatches, payload.productBatches,
    metadata.items, metadata.products, metadata.batches, metadata.expiryItems, item?.items, groupItems,
  ];
  const rows = candidates.find((value) => Array.isArray(value) && value.length > 0);
  if (Array.isArray(rows)) return rows;
  if (firstDefined(payload.productName, payload.batchNo, payload.expiryDate, payload.skt, payload.sourceKey)) return [payload];
  return [];
}

function getExpiryDetailRows(item) {
  return getExpiryDetailSourceRows(item).map((row, index) => normalizeExpiryDetailRow(row, index));
}

function isExpiryNotification(item) {
  const payload = getNotificationPayload(item);
  const haystack = [item?.type, item?.title, item?.description, item?.message, item?.actionType, item?.category, payload.groupReason, payload.groupLabel]
    .filter(Boolean)
    .join(' ')
    .toLocaleLowerCase('tr-TR');
  return /skt|son kullanma|expiry|expired|expiration/.test(haystack);
}

function getExpiryDetailSummary(item, rows = []) {
  const payload = getNotificationPayload(item);
  const uniqueProducts = new Set(rows.map((row) => [row.productName, row.sku, row.barcode].join('|')));
  return {
    affectedProductCount: Number(firstDefined(payload.affectedProductCount, payload.productCount, payload.totalProducts, uniqueProducts.size, rows.length) || 0),
    affectedBatchCount: Number(firstDefined(payload.itemCount, payload.batchCount, payload.totalBatches, rows.length) || 0),
    priority: PRIORITY_LABELS[item?.priority] || item?.priority || '-',
    module: getNotificationModuleLabel(item),
    createdAt: formatAbsoluteDateTime(item?.createdAt),
  };
}

function getExpiryDetailDescription(item, rows = []) {
  const summary = getExpiryDetailSummary(item, rows);
  if (summary.affectedProductCount || summary.affectedBatchCount) {
    return `${summary.affectedProductCount} ürüne ait ${summary.affectedBatchCount} parti kaydının SKT tarihi geçmiş durumda. Aşağıdaki listeden ürün, parti ve stok adetlerini inceleyebilirsiniz.`;
  }
  return 'SKT tarihi geçmiş ürün ve parti kayıtlarını inceleyin.';
}
...
export default function Notifications() {
  ...
  const detailIsExpiry = Boolean(detailModalItem && isExpiryNotification(detailModalItem));
  const detailExpiryRows = useMemo(() => (detailModalItem ? getExpiryDetailRows(detailModalItem) : []), [detailModalItem]);
  const detailExpirySummary = useMemo(() => (detailModalItem ? getExpiryDetailSummary(detailModalItem, detailExpiryRows) : null), [detailModalItem, detailExpiryRows]);
  const filteredExpiryRows = useMemo(() => {
    const query = String(detailSearch || '').trim().toLocaleLowerCase('tr-TR');
    const source = detailExpiryRows;
    if (!query) return source;
    return source.filter((row) => [row.productName, row.sku, row.barcode, row.batchNo].some((value) => String(value || '').toLocaleLowerCase('tr-TR').includes(query)));
  }, [detailExpiryRows, detailSearch]);
  const visibleExpiryRows = filteredExpiryRows.slice(0, 50);
    ...
    if (searchQuery) {
      rows = rows.filter((item) => {
        return [
          ...
          .filter(Boolean)
          .some((val) => includesNormalized(val, searchQuery));
      });
    }
  ...
  const searchedAudienceUsers = useMemo(() => {
    ...
    if (!audienceSearch) return source;
    return source.filter((item) => {
      return [item.name, item.username, item.email, item.department, formatRoleLabel(item.role)]
        .filter(Boolean)
        .some((val) => includesNormalized(val, audienceSearch));
    });
  ...
  return (
    <div className="page-stack notification-page">
      ...
      <FormModal
        ...
        title={detailIsExpiry ? 'SKT Geçmiş Ürünler' : (detailModalItem?.title || 'Bildirim Detayı')}
        description={detailIsExpiry ? 'SKT tarihi geçmiş ürün ve parti kayıtlarını inceleyin.' : 'Bildirim içeriğini, kaynağını ve referans bilgilerini tek panelde inceleyin.'}
        headerIcon={<Info size={16} />}
        onClose={() => { setDetailModalItem(null); setDetailSearch(''); }}
        modalClassName="access-request-detail-modal notification-order-draft-modal notification-detail-modal notification-expiry-detail-modal modal-header-standardized"
        ...
        {detailModalItem ? (
          <div className="modal-form notification-order-draft-form">
            <div className="modal-form-body-scroll access-request-detail-body notification-order-draft-body">
              <div className="access-request-detail-grid notification-detail-summary-grid">
                <div><span>{detailIsExpiry ? 'Etkilenen ürün sayısı' : 'Başlık'}</span><strong>{detailIsExpiry ? detailExpirySummary?.affectedProductCount : (detailModalItem.title || '-')}</strong></div>
                <div><span>{detailIsExpiry ? 'Etkilenen parti sayısı' : 'Tür'}</span><strong>{detailIsExpiry ? detailExpirySummary?.affectedBatchCount : (detailModalItem.type || detailModalItem.actionType || '-')}</strong></div>
                <div><span>Öncelik</span><strong>{detailIsExpiry ? detailExpirySummary?.priority : (PRIORITY_LABELS[detailModalItem.priority] || detailModalItem.priority || '-')}</strong></div>
                <div><span>İlgili modül</span><strong>{detailIsExpiry ? detailExpirySummary?.module : getNotificationModuleLabel(detailModalItem)}</strong></div>
                <div><span>Oluşturulma zamanı</span><strong>{detailIsExpiry ? detailExpirySummary?.createdAt : formatAbsoluteDateTime(detailModalItem.createdAt)}</strong></div>
              ...
              <article className="access-request-detail-note">
                <h4>Açıklama</h4>
                <p>{detailIsExpiry ? getExpiryDetailDescription(detailModalItem, detailExpiryRows) : (detailModalItem.description || detailModalItem.message || 'Detay açıklaması bulunmuyor.')}</p>
              </article>

              {detailIsExpiry ? (
                <article className="access-request-detail-note notification-expiry-table-section">
                  <div className="notification-expiry-section-head">
                    <div>
                      <h4>Ürün ve Parti Listesi</h4>
                      <p>{filteredExpiryRows.length > 50 ? 'İlk 50 kayıt gösteriliyor. Toplam ' + filteredExpiryRows.length + ' kayıt bulundu.' : filteredExpiryRows.length + ' kayıt gösteriliyor.'}</p>
                    </div>
                    <InputWithIcon
                      className="notification-expiry-search"
                      icon={<Search size={15} />}
                      id="notification-expiry-detail-search"
                      type="text"
                      value={detailSearch}
                      onChange={(event) => setDetailSearch(event.target.value)}
                      placeholder="Ürün adı, SKU, barkod veya parti no ara"
                    />
                  </div>
                  {visibleExpiryRows.length > 0 ? (
                    <div className="notification-expiry-table-wrap">
                      <table className="notification-expiry-table">
                        <thead>
                          <tr>
                            <th>Ürün adı</th>
                            <th>SKU</th>
                        ...
                        <tbody>
                          {visibleExpiryRows.map((row, index) => (
                            <tr key={row.key + '-' + index}>
                              ...
                              <td>{row.shelfStock}</td>
                              <td>{row.totalStock}</td>
                              <td><span className={'notification-expiry-status is-' + (row.status === 'Geçmiş' ? 'expired' : row.status === 'Bugün' ? 'today' : 'soon')}>{row.status}</span></td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                    ...
                    <div className="notification-empty notification-expiry-fallback">
                      Bu bildirim için ürün/parti detay listesi bulunamadı. Detaylı listeyi SKT Takibi sayfasından görüntüleyebilirsiniz.
                    </div>
                  )}
                </article>
                ...
                <article className="access-request-detail-note">
                  ...
                  <div className="notification-order-draft-list">
                    {getNotificationGroupItems(detailModalItem).slice(0, 50).map((row, index) => (
                      <div key={String(row.sourceKey || row.productId || row.sku || index)} className="notification-order-draft-line">
                    ...
                    {getNotificationGroupItems(detailModalItem).length === 0 ? <div className="notification-empty">Bu bildirim için detay listesi bulunamadı.</div> : null}
                  </div>
                </article>
              ) : (
                <article className="access-request-detail-note">
                  <h4>Detay</h4>
                  <p>Bu bildirim için ek kullanıcı detayı bulunmuyor.</p>
                </article>
              )}
            </div>
            <div className="modal-actions notification-order-draft-footer">
              {detailIsExpiry ? <button type="button" className="ghost-button" onClick={() => navigate('/skt-takibi')}>SKT Takibi Sayfasına Git</button> : null}
              ...
              <button type="button" className="secondary-button" onClick={() => { setDetailModalItem(null); setDetailSearch(''); }}>Kapat</button>
            </div>
          </div>
        ) : null}
      </FormModal>
