...
export default function Reports() {
  ...
  const filteredProductOptions = useMemo(() => {
    const search = String(salesReturnFilters.productSearch || '').trim().toLocaleLowerCase('tr-TR');
    ...
    return normalizedProducts
      .filter((product) => [product.name, product.sku, product.barcode].filter(Boolean).join(' ').toLocaleLowerCase('tr-TR').includes(search))
      .slice(0, 120);
