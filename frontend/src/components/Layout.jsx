...
const ROUTE_PAGES = [
  { path: '/anasayfa', page: 'Dashboard' },
  { path: '/urunler', page: 'Ürünler' },
  { path: '/kategoriler', page: 'Kategoriler' },
  { path: '/eslesmeler', page: 'Eşleşmeler' },
  { path: '/lokasyon-yonetimi', page: 'Lokasyon Yönetimi' },
  { path: '/stok-islemleri', page: 'Stok İşlemleri' },
  { path: '/skt-takibi', page: 'SKT Takibi' },
  { path: '/tedarikciler', page: 'Tedarikçiler' },
  { path: '/siparis-olustur', page: 'Sipariş Oluştur' },
  { path: '/siparis-takibi', page: 'Sipariş Takibi' },
  { path: '/fiyat-talep-analizi', page: 'Fiyat & Talep Analizi' },
  { path: '/kampanya-yonetimi', page: 'Kampanya Yönetimi' },
  { path: '/siparis-onerileri', page: 'Sipariş Önerileri' },
  { path: '/personel-yonetimi', page: 'Personel Yönetimi' },
  { path: '/musteri-yonetimi', page: 'Müşteri Yönetimi' },
  { path: '/rol-yonetimi', page: 'Rol Yönetimi' },
  { path: '/erisim-taleplerim', page: 'Taleplerim' },
  { path: '/erisim-talepleri', page: 'Erişim Talepleri' },
  { path: '/proximity-yonetimi', page: 'Proximity Yönetimi' },
  { path: '/etiket-yonetimi', page: 'Etiket Yönetimi' },
  { path: '/nasil-kullanilir', page: 'Nasıl Kullanılır' },
  { path: '/sistem-ayarlari', page: 'Ayarlar' },
  { path: '/pos-kasa', page: 'POS / Kasa' },
  { path: '/depo-transfer-talepleri', page: 'Depo Transfer Talepleri' },
  { path: '/gorev-planlama', page: 'Görev Planlama' },
  { path: '/raporlar', page: 'Raporlar' },
  { path: '/bildirimler', page: 'Bildirimler' },
];
...
export default function Layout() {
  ...
  const matchedRoute = useMemo(() => {
    return ROUTE_PAGES.find(item => location.pathname === item.path || location.pathname.startsWith(`${item.path}/`));
  }, [location.pathname]);

  const isBlockedByPlan = useMemo(() => {
    if (!matchedRoute) return false;
    if (matchedRoute.page === 'Nasıl Kullanılır') return false;

    const screenAccess = Array.isArray(user?.screenAccess) ? user.screenAccess : [];
    if (screenAccess.length === 0) return false;

    if (!screenAccess.includes(matchedRoute.page)) {
      if (matchedRoute.page === 'Sipariş Takibi' && screenAccess.includes('Sipariş Oluştur')) {
        return false;
      }
      return true;
    }
    return false;
  }, [matchedRoute, user?.screenAccess]);
          ...
          {isBlockedByPlan ? (
            <section className="page-access-blocked" aria-live="polite">
              <div className="page-access-blocked-card">
                <h2>Bu ekran mevcut lisans planınızda aktif değil.</h2>
              </div>
            </section>
