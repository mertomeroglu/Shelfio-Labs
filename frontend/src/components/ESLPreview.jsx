...
function drawBarcode(ctx, rawCode, x, y, width, height) {
  const code = String(rawCode || '0000000000000').replace(/\D/g, '') || '0000000000000';
  const quietModules = 2;
  const moduleW = Math.max(1, Math.floor(width / 110));
  const totalModules = Math.min(95, Math.max(44, code.length * 7));
  const barcodeW = (totalModules + quietModules * 2) * moduleW;
  let bx = x + Math.max(0, Math.floor((width - barcodeW) / 2)) + quietModules * moduleW;

  ctx.fillStyle = '#000000';
  for (let i = 0; i < totalModules; i += 1) {
    const digit = Number(code[i % code.length] || 0);
    const isGuard = i < 3 || (i >= 45 && i <= 49) || i > totalModules - 4;
    const isBar = isGuard ? i % 2 === 0 : ((digit + i) % 3 !== 1);
    if (isBar) ctx.fillRect(bx, y, moduleW, height);
    bx += moduleW;
  }
...
function drawStandardLabel(ctx, data) {
  ...
  const blockBottomY = bcY - 6;
  const fdtY = blockBottomY - 10;
  const originY = fdtY - 4 - 10;
...
function drawCampaignLabel(ctx, data) {
  ...
  if (data.fdt) {
    ctx.textAlign = 'right';
    ctx.fillText(data.fdt, boxX + boxW - 5, boxY + 9);
  }
}
