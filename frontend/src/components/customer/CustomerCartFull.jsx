...
export default function CustomerCartFull({
  ...
  const hasOpenModal = Boolean(confirmList || confirmCheckout || confirmClear);
  ...
  return (
    <div className="customer-subpage customer-cart-full customer-cart-full-v2">
      ...
      {confirmList ? (
        <ConfirmActionModal
          title="Alışveriş Listesi Onayı"
          description="Seçili ürünleri alışveriş listene eklemek istiyor musun?"
          confirmLabel="Listeye Ekle"
          tone="info"
          onCancel={() => setConfirmList(false)}
          onConfirm={() => {
            onAddToShoppingList(selectedIds);
      ...
      {confirmCheckout ? (
        <ConfirmActionModal
          ...
          description="Siparişi tamamlamak istiyor musun?"
          confirmLabel="Siparişi Tamamla"
          tone="success"
