...
import { AlertTriangle, Barcode, Bell, Boxes, Calendar, CheckCheck, CircleAlert, Clock, Info, LogOut, Menu, MessageCircle, Search, Settings2, Store, X } from 'lucide-react';
...
export default function Header({ onMenuClick, settings, onOpenSupport }) {
  ...
  const runGlobalSearch = async (rawValue) => {
    ...
    const normalizedQuery = normalizedInput.toLocaleLowerCase('tr-TR');
    ...
    try {
      setSearching(true);
      const normalizeSearchText = (text) => String(text || '').toLocaleLowerCase('tr-TR').trim();
      const tokenStartsWithQuery = (text) => {
        const normalizedText = normalizeSearchText(text);
        if (!normalizedText || !normalizedQuery) return false;
        if (normalizedQuery.includes(' ')) return normalizedText.includes(normalizedQuery);
        const tokens = normalizedText.split(/[^0-9a-zçşıöşü]+/i).filter(Boolean);
      ...
      const filtered = {
        products: safeData.products.filter((item) => (
          tokenStartsWithQuery(item?.name)
          || tokenStartsWithQuery(item?.sku)
          || normalizeSearchText(item?.barcode).includes(normalizedQuery)
        )),
        categories: safeData.categories.filter((item) => tokenStartsWithQuery(item?.name)),
        suppliers: safeData.suppliers.filter((item) => tokenStartsWithQuery(item?.name)),
      };
