...
const AUTH_CLEAR_ERROR_CODES = new Set([
  'missing_token',
  'invalid_token',
  'user_not_found',
]);

export const shouldClearAuthForError = (error) => {
  const status = Number(error?.status || error?.statusCode || 0);
  const errorCode = String(error?.payload?.errorCode || error?.errorCode || '').trim();
  if (AUTH_CLEAR_ERROR_CODES.has(errorCode)) return true;
  if (status === 401 && !errorCode) return true;
  return false;
};
...
const queueDeveloperLog = (payload) => {
  ...
  if (!token) return;
  const logEndpoint = '/settings/developer-logs';
  ...
  pendingStaffRefreshPromise = fetch(`${API_BASE_URL}/auth/refresh`, {
    ...
    .then(async (response) => {
      ...
      const sessionUser = {
        ...
        activeStore: data.activeStore || data.user?.activeStore || null,
        license: data.license || data.user?.license || null,
        licenseSummary,
        plan: data.plan || data.user?.plan || null,
        enabledModules: data.enabledModules || data.user?.enabledModules || [],
        screenAccess: data.screenAccess || licenseSummary?.screenAccess || data.user?.screenAccess || [],
        permissions: data.permissions || data.user?.permissions,
      };
      const sessionData = { ...data, user: sessionUser };
      setAuthToken(data.token);
      setAuthRefreshToken(data.refreshToken);
      setStoredUser(sessionUser);
      clearSessionCache();
      notifyAuthRefreshed(sessionData);
      return sessionData;
    })
    .catch((error) => {
      clearAuthToken();
...
async function request(path, options = {}) {
  ...
  if (!response.ok) {
    ...
    const rawFriendlyMessage = response.status === 401
      ...
      : response.status === 403
        ? (['module_not_licensed', 'screen_not_licensed', 'module_access_denied', 'screen_access_denied'].includes(payload?.errorCode)
          ? 'Bu ekran mevcut lisans planınızda aktif değil.'
          : /^Bu işlem için .+ izni gerekli$/i.test(String(payload?.message || ''))
            ? 'Bu işlem için yetkiniz bulunmuyor.'
            : payload?.message || 'Bu işlem için yetkiniz bulunmuyor.')
    ...
    if (shouldClearAuthForError({ status: response.status, payload })) {
      clearAuthToken();
      notifyAuthExpired();
    }
